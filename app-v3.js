...
$("connect-ml").addEventListener("click", async () => {
  $("ml-result").textContent = "Gerando autorização...";
  const { data, error } = await client.functions.invoke("ml-connect");
  if (error || data?.error) {
    $("ml-result").textContent = error?.message || data.error;
    return;
  }
  window.location.href = data.authorization_url;
});

$("diagnostic-ml").addEventListener("click", async () => {
  $("ml-result").textContent = "Testando conexão...";
  const { data, error } = await client.functions.invoke("ml-diagnostic");
  $("ml-result").textContent = JSON.stringify(error ? { error: error.message } : data, null, 2);
});
